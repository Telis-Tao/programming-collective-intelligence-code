...
def hcluster(vecs, distance=pearson):
    ...
    while len(clust) > 1:
        ...
        error = closest
        newcluster = bicluster(mergevec, left=clust[lowestpair[0]], right=clust[lowestpair[1]], distance=error,
                               id=currentclustid)
...
def kcluster(vecs, distance=pearson, k=4):
    ...
    clusters = [[random.random() * (ranges[i][1] - ranges[i][0]) + ranges[i][0] for i in range(len(vecs[0]))] for j in
                range(k)]
